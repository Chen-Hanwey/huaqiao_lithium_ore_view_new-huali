...
import $ from "jquery";
import { listAllOperation, listOperationManageTreeWithDept } from '@/api/operationManage/operationManage'
...
export default {
  ...
  methods: {
    ...
    getAllOperationList(){
      listAllOperation({}).then(res=>{
        this.operationList = res.data
      })
    },
    getAllEnterpriseList(){
      listEnterpriseTreeWithDept("03").then(res=>{
        this.enterpriseList=res.data;
      })
    },
    getRole() {
      getRoleByUserType("04").then(response => {
    ...
    getList() {
      ...
      listSupplierUser(this.addDateRange(this.queryParams, this.dateRange)).then(response => {

import { listMonth, getMonth, delMonth, addMonth, updateMonth, exportMonth } from "@/api/project/month/month";
import $ from "jquery";
...
export default {
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      this.queryParams.params.beginMonyh = this.queryParams.monyhRange && this.queryParams.monyhRange.length > 0 ? this.queryParams.monyhRange[0] : undefined;
      this.queryParams.params.endMonyh = this.queryParams.monyhRange && this.queryParams.monyhRange.length > 0 ? this.queryParams.monyhRange[1] : undefined;
    ...
    openDialog () {
      $('.month-body').parent().css('height', $(window).innerHeight() - 100).css('top', 0).css('overflow-y', 'auto');
    },
    /** 提交按钮 */
    submitForm: function () {
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除月度计划编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出所有月度计划数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {

import { listPurchaseOut, getPurchaseOut, delPurchaseOut, addPurchaseOut, updatePurchaseOut, exportPurchaseOut } from "@/api/purchaseManage/purchaseOut";
import $ from "jquery";
...
export default {
  name: 'PurchaseOut',
  components: {purchaseInventoryDialog,OperationDialog},
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        ...
        operationName: [
          { required: true, message: "运营场所不能为空", trigger: "change" }
        ]
      },
      columnLabel: ''
    };
  ...
  methods: {
    ...
    operationCallBack(msg) {
      ...
      console.log(data)
      if (data.length > 0) {
        this.form.distributeOperation = data[0].id;
        this.operationName = data[0].name;
        this.$set(this.form,'operationName',data[0].name)
      } else {
        this.form.distributeOperation = undefined;

import {
  ...
  addStorageRecords
} from "@/api/purchaseManage/purchaseStorage";
import $ from "jquery";
import {getToken} from "@/utils/auth";
import purchaseRecordDialog from "@/views/purchaseManage/components/purchaseRecordDialog";
...
export default {
  ...
  components: {purchaseRecordDialog},
  data() {
    return {
      // 遮罩层
      loading: true,
    ...
    callBack(val) {
      getPurchaseRecord(val.id).then(response => {
        this.$nextTick(()=>{
          ...
          this.form.purchaseRecordId = this.form.id
          this.form.id = null
